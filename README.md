...
| [`/common‑rules`](#common-rules) | 激活通用行为规范（任务摘要、人工待办按 @角色 分派、飞书卡片 2.0 单栏通知（白名单：代码实现/Bug 修复/测试/新增功能/Bug 转 case/生成待办）、v0 文档只读保护、禁止硬编码、commit 格式、CORS 走 nginx、三环境配置对齐、API 安全基线） |
...
| [`/git‑summary`](#git-summary) | 把分支自创建时间点以来的 commit message 归并加工成功能清单（拆句→去噪→按功能域归并→新增/修复/口径变更三段→每条挂 commit hash）；区间起点只有 `--base`/`--since`/分支创建点三种，不做版本族上探下探、不换基线，空增量输出区间性质判定而非空清单，默认当前分支，`--raw` 出原始列表，全程只读 |
...
- 通知一律为**卡片 JSON 2.0 单栏版式**（`msg_type=interactive`、`schema: "2.0"`），禁止自由文本段落，
  **禁止 `column_set` 分栏**（实测视觉不佳）。区块顺序：标题（状态图标 + 配色 green/orange/red/blue）
  → 时间三行（开始 / 结束 / 耗时，每行一项）→ `**任务结果**` → `**影响范围**` →
  `**人工待办**`（逐条 `@角色`，无待办写 `1. 无`）→ 灰字脚注（工程名，2.0 已不支持 `note` 元素）
- 正文一律用 `markdown` 元素承载（**不进代码块**）：小标题渲染为**加粗**，条目 `1.` `2.` `3.`
  渲染为带悬挂缩进的有序列表；代价是长行按卡片宽度自动折行，因此每条摘要条目只写一句话说清
- 卡片正文与摘要**逐字一致**，保留换行、路径、编号与标点，禁止精简、合并、截断或写"等 N 个"
  "详见摘要"。仅允许三处承载形式差异：标题栏简短化；小标题符号 `### X` ↔ `**X**`、时间行
  一行 ↔ 三行；卡片内省略 `飞书通知` 状态行（发送前不可知）
- 内容过长时按摘要原顺序拆成多个 `markdown` 元素；整卡超出飞书长度上限则拆成多条卡片依次发送并在标题
  标注 `(1/2)`、`(2/2)`，一字不减
- **失败诊断**：发送脚本必须同时打印 `code` 与 `msg`。卡片是整体校验，**任一元素不合法就整卡被拒**，外层只返回 `code=11246`，真因在 `msg` 的 `ErrCode` / `ErrPath` / `ErrorValue` 三元组里（典型：`200861 unsupported tag note`——2.0 已废弃 `note`）；只打印 `code` 会把它误判成"卡片没生效/宽度问题"
- **回退禁止跳级**：结构类错误（`elements` 未嵌在 `body` 下、用了 `horizontal_line` 或 `note` 等）必须先按 `ErrPath` 定位并修正元素后重发卡片；只有修正后仍失败，才降级为同字段顺序的固定行文本（同样照搬原文）。摘要 `飞书通知` 记失败原因时带上 `ErrCode`
...
| 参数 | 说明 |
|---|---|
| `--bug-id` | 指定模式：跳过 `/bugs/next` 队列，只处理这一条并按该 `#bugId` 回写状态（正整数，非法直接报错；不限原状态，但会覆盖原状态，处理完即结束） |
...
| `--since` | 区间起点，**只接受时间值**：`YYYY-MM-DD` / `YYYY-MM-DDTHH:MM` / `今天`/`昨天`/`前天`（tag / 分支起点用 `--base`）。时间窗口口径，含同期并入的基线提交，且按 committer date 过滤 |
| `--limit` | **区间内最近 N 条提交**参与加工（不是功能项条数） |
| `--raw` | 输出原始逐条列表与统计，不做归并（怀疑加工失真时用它核对底稿） |
| `--domain-map` | 自定义功能域词典（每行一个域名），覆盖内置 14 域 |
| `--with-merges` | 加工时包含 merge 提交（默认排除） |

**区间起点（只有三种来源，没有第四种）**：`--base=<ref>` → `--since=<日期>` → 分支创建点 `merge-base(默认基线, 分支)`（默认口径）。**不做上探 / 下探**：不沿版本族替换基线（`feat-1.4.7.5 → .4 → .3` 这类），不自动改用 tag 或别的分支当起点，不因区间只有几条提交就改口径——区间窄就照实输出几条，取舍交给读者。0 条或无功能增量时输出「区间性质判定 + 候选建议」（候选分支/tag 及其提交数只作建议，必须显式 `--base` 才生效），**不会自动展开整条仓库历史**。
...
**输出**：`## 功能清单：<分支>（区间 <起>..<止>，基线来源：<来源>，N 条提交 → M 个功能点）` + 三段清单（每项 `**功能名**：说明（hash, hash）`，按重要度排序，影响资金/履约/权限的在前）+ `> 未列入：…` 一行 + `总结：…` + `起止：<开始> ~ <结束>（东八区）` 一行。

「总结」是固定槽位，格式 `总结：<要点 1；要点 2；要点 3>（N 条提交 → M 个功能点，新增 x / 修复 y / 口径变更 z）`：**主体是产品功能与 bug 修复要点**——从清单已渲染项里按重要度挑最多 3 条，优先修复类与影响资金/履约/权限的功能，只能挑不能新造，保留表名/错误码/接口路径原词；计数退到末尾括号（三段条数是清单渲染的**功能项数**，不是 commit 前缀计数）。上限 2 行 / 120 字，不评价质量、不提建议；`M = 0` 与 `--raw` 时不输出该段。「基线来源」是固定槽位（`用户指定（--base=<ref>）`/`用户指定（--since=<日期>）`/`分支创建点（merge-base）`），只能是这三者之一，写在括号内，禁止在括号外补解释。功能点为 0、区间 0 条、或区间内无任何 `feat:`/`fix:` 与写出外部行为变化的提交时**不渲染空三段模板**，改出「区间性质判定」：一行定性 + 提交构成（feat/fix/test/docs 各多少条）+ 实测候选建议及其提交数（须显式 `--base` 才生效，本 skill 不自动换基线）。功能项超过 60 条截断并提示用 `--limit` 收窄。本 skill 属只读汇总类任务，尾注即这一行起止时间，不输出规范一的其余三块。
