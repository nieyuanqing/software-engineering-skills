...
**区间两个锚点（唯一口径，没有第三个参数）**：**终点锚**＝在主干 first-parent 链上按从旧到新扫描，第一个已经包含分支 tip 的 merge 提交，即该分支最近一次有效并入（分批并入过多次时就是最后一次带进新提交的那次；判定方向必须是 `tip ⊆ merge`，反了会把「本分支创建之前刚并入的别的分支」认成本分支的并入点）；扫不到（尚未并入，或主干走 squash / rebase 没有 merge 提交）就取分支 tip。**起点锚**＝已并入时用 `merge-base(并入merge^1, tip)`——直接 `merge-base(main, tip)` 会被并入动作污染成分叉点＝tip，区间恒为 0 条；未并入时用 `merge-base(main, tip)`。不沿版本族上探 / 下探、不自动改用别的分支或 tag 当锚点，区间只有几条也照实输出；0 条或无功能增量时输出「区间性质判定」，**不展开整条仓库历史**。
