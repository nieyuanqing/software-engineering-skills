...
| [`/git‑summary`](#git-summary) | 把分支从创建点到并入主干（未并入则到分支 tip）的 commit message 归并加工成功能清单（拆句→去噪→按功能域归并→新增/修复/口径变更三段→每条挂 commit hash→要点式总结）；区间只有两个锚点、不给手工指定区间的参数，不做版本族上探下探，空增量输出区间性质判定而非空清单，默认当前分支，`--raw` 出原始列表，全程只读 |
...
把指定分支的 commit message 归并加工成一份**功能清单**，而不是逐条罗列提交。区间只有两个锚点：**分支创建点 → 该分支最近一次并入 main 的 merge**（还没并入就取分支 tip）；默认目标分支为当前分支，全程只读，不改动任何 git 状态；要原始逐条列表用 `--raw`。

**用法**

```bash
/git-summary                                  # 当前分支：创建点 → 并入主干（或 tip）
/git-summary feat-1.4.7.5                     # 指定分支，位置参数等价 --branch
/git-summary --branch=origin/feat-1.4.8       # 远端分支
/git-summary --raw                            # 逐条原始列表，核对加工是否失真
...
**区间两个锚点（唯一口径，没有第三个参数）**：**起点锚**三级优先——① 本机 `git reflog` 中该分支的 `branch: Created from` 记录（建分支那一刻的提交，最可靠，不受之后是否并入影响；只存在于创建过该分支的那台机器）；② 有并入 merge 时取 `merge-base(并入merge^1, tip)`；③ 否则 `merge-base(main, tip)`，且**必须校验它不等于终点锚**。**终点锚**三种性质必须区分：主干 first-parent 链上扫到第一个已包含 tip 的 merge → `并入 main 的 merge`；tip 已在主干但扫不到 merge（整仓库 fast-forward，主干没有 merge 提交）→ `已并入 main（fast-forward / 无 merge 提交）`；tip 不在主干 → `分支 tip（仍有提交未并入）`。扫 merge 方向必须是 `tip ⊆ merge`，反了会把「本分支创建前刚并入的别的分支」认成自己的并入点。**三级起点全取不到时走「区间性质判定出口」并写明创建锚取不到，禁止输出两端同一个 hash 的区间**；不沿版本族上探 / 下探、不自动改用别的分支或 tag 当锚点，区间只有几条也照实输出，**不展开整条仓库历史**。
