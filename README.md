...
| [`/git‑summary`](#git-summary) | 把分支自创建时间点以来的 commit message 归并加工成功能清单（拆句→去噪→按功能域归并→新增/修复/口径变更三段→每条挂 commit hash），基线走推断时校验区间规模（不足 8 条提交且跨度不到 1 天才判过窄、沿同族上探），空增量输出区间性质判定而非空清单，默认当前分支，`--raw` 出原始列表，全程只读 |
...
**区间解析优先级**：`--base` → `--since` → `merge-base(默认基线, 分支)`（默认口径＝分支创建点）→ 同族上一版分支（`feat-1.3.6` → `feat-1.3.5`，按 `sort -V` 取紧邻前一版）→ 最近一个 tag。**走退化链得到的基线是推断值，必须实测区间规模**：只有「不足 8 条提交 **且** 跨度不到 1 个自然日」才算区间过窄（14 条 / 19 天、3 条 / 2 天的 hotfix 都算正常增量，不上探），过窄时沿同族继续往前上探（`.5 → .4 → .3`，最远到去掉末段版本号的族主干 `feat-1.4.7.5 → feat-1.4.7`），避免把一次完整迭代截成几条收尾台账。全落空则输出「区间性质判定 + 候选区间」，**不会自动展开整条仓库历史**。
...
**输出**：`## 功能清单：<分支>（区间 <起>..<止>，基线来源：<来源>，N 条提交 → M 个功能点）` + 三段清单（每项 `**功能名**：说明（hash, hash）`，按重要度排序，影响资金/履约/权限的在前）+ `> 未列入：…` 一行 + `起止：<开始> ~ <结束>（东八区）` 一行。「基线来源」是固定槽位（`用户指定`/`分叉点`/`同族上一版 X`/`同族上探 X`/`tag X`），推断基线只能由该字段表达，禁止在括号外补解释。功能点为 0、或区间内无任何 `feat:`/`fix:` 与写出外部行为变化的提交时**不渲染空三段模板**，改出「区间性质判定」：一行定性 + 提交构成（feat/fix/test/docs 各多少条）+ 实测候选区间及其提交数与重跑命令。功能项超过 60 条截断并提示用 `--limit` 收窄。本 skill 属只读汇总类任务，尾注即这一行起止时间，不输出规范一的其余三块。
