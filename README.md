...
- 通知一律为**卡片 JSON 2.0 单栏版式**（`msg_type=interactive`、`schema: "2.0"`），禁止自由文本段落，
  **禁止 `column_set` 分栏**（实测视觉不佳）。区块顺序：标题（状态图标 + 配色 green/orange/red/blue）
  → 时间三行（开始 / 结束 / 耗时，每行一项）→ 一个 ```` ```text ```` 代码块（内含摘要
  `### 任务结果`、`### 影响范围`、`### 人工待办` 三块原文，待办逐条 `@角色`、无则写"- 无"）
  → 灰字脚注（工程名，2.0 已不支持 `note` 元素）
- 摘要原文整体放代码块的原因：PC 端长行**横向滚动不折行**，避免窄卡片把文件路径与条目切碎
  （移动端仍会折行，属客户端限制）
- 代码块内正文与摘要**逐字一致**，保留换行、路径、编号、标点与 `### ` 小标题行，禁止精简、合并、
  截断或写"等 N 个""详见摘要"（仅标题栏可简短化，代码块内省略 `飞书通知` 状态行）
- 内容过长时按摘要原顺序拆成多个代码块；整卡超出飞书长度上限则拆成多条卡片依次发送并在标题
  标注 `(1/2)`、`(2/2)`，一字不减
