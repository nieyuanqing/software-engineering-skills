...
- **「基线来源」是表头固定槽位**，取值只能是 `用户指定（--base=<ref>）` / `用户指定（--since=<ref>）` /
...
- **`--raw` 模式**改为输出原始列表与统计：表头（分支、基线、分叉点、区间起始时间、提交数、时间跨度、作者数、类型分布）+ 逐条 `短 hash 日期 首行`（首行超 72 字符截为 71 + `…`）。分支即基线时区间为空，按第二节走区间性质判定出口，不展开整条历史、不换基线。
