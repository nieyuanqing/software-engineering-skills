...
推断出的基线须校验区间规模：不足 8 条提交且跨度不到 1 个自然日才算"区间过窄"，此时沿同族继续上探
到族主干（feat-1.4.7.5 → feat-1.4.7）。仍无功能增量时输出「区间性质判定 + 候选区间」，不渲染空清单，
也不会自动展开整条仓库历史。
...
5. **推断基线的规模下限校验（走 4a/4b 时必做）**：退化链拿到的基线是**猜的**，选中后必须实测一次区间规模——
   `git rev-list --count "$RANGE"` + 区间首末条 author date 跨度。
   **只有提交数与跨度同时偏低才判为"区间过窄"：`< 8 条提交` 且 `跨度 < 1 个自然日`**——同一批收尾工作被压成
   几小时内的几条台账，正是这种形态。只满足其一就算正常增量、**不上探**：14 条 / 19 天是一次完整迭代；
   3 条 / 2 天的 hotfix 本来就小，但确实是独立一轮改动。
   - 判为过窄 → **继续沿同族往前上探**（`.5 → .4 → .3 …`），取第一个脱离该判定的基线，最远上探到"去掉末段
     版本号的族主干"（`feat-1.4.7.5` → `feat-1.4.7`）为止。同一批工作被切成 `.4` / `.5` 两个分支是常态，
     只取紧邻前一版会把一次完整迭代截成几条台账提交。
   - 全部候选仍过窄 → **不硬渲清单**，改走第四节的「区间性质判定出口」，其中列出各候选区间及其提交数供用户重跑。
   - `--base` / `--since` 显式指定的基线**不做**该校验（区间口径由用户给定），但表头「基线来源」必须写
     `用户指定`。
...
- **「基线来源」是表头固定槽位**，取值只能是 `用户指定`（`--base`/`--since`）、`分叉点`（merge-base）、
  `同族上一版 <分支名>`、`同族上探 <分支名>`（走过第二节第 5 条上探）、`tag <tag名>`。
  基线是推断出来的这件事**只能由该字段表达**，禁止在括号外另挂一句解释（"不解释过程、形状必须一致"含此意）：
  上探过一轮就写 `同族上探 feat-1.4.7`，不写"因为 .4 只差 5 条所以换成 .7"。
...
- 入选项功能点 `M = 0`；或
- 区间内没有任何 `feat:` / `fix:` 前缀、也没有写出外部行为变化的提交（全是 `test:` / `docs:` / 台账类，按 3.2 封顶后必然为 0）；或
- 按第二节第 5 条上探到族主干后，区间仍属"不足 8 条提交 **且** 跨度不到 1 个自然日"的过窄形态。

```
## 区间性质：<BRANCH>（区间 <起短hash>..<止短hash>，基线来源：<来源>，N 条提交 → 0 个功能点）

本区间为 <一句话定性：迭代收尾台账 / 纯测试取证 / 区间过窄>，无产品功能增量。
提交构成：feat a 条 / fix b 条 / test c 条 / docs d 条 / 其它 e 条

候选区间（更大口径重跑，括号内为实测提交数）：<候选分支或 tag（实测 x 条）>
需要功能清单：/git-summary --base=<候选>

起止：<开始时间> ~ <结束时间>（东八区）
```

- 候选区间与其提交数必须来自本次实测（`git branch --format` + `git rev-list --count`）；无可给候选时写
  `候选区间：无（同族已探到根）`，禁止编造分支名或条数。
- 该出口只说明"这个区间没有功能增量、要看更大区间请这么跑"，**不展开整条历史、不另铺别的区间清单**。
...
5. 输出功能项数不得超过拆出的子句总数；每条必须可 hash 溯源；无增量时走第四节「区间性质判定出口」输出定性 + 候选区间，不铺整条历史。
