...
description: 为 Java/Spring Boot 工程生成完整的标准化部署配置：deploy.sh、apply-ssl.sh、手工连库入口 db-sql.sh（默认只读、写需 --apply）、Flyway 迁移目录（src/main/resources/db/migration）、nginx 站点配置（dev/test/prod 三套）、三套键集对齐的 .env、Spring Boot 配置与健康检查端点、specs/deployment.md、标准 .gitignore（含 env 与 SQL 文件忽略）、sql 备份目录。数据库结构由 Flyway 在应用启动时自动迁移（版本记账用 flyway_schema_history，Hibernate 只 validate）。supervisord 配置由 deploy.sh 部署时 inline 生成。所有产物遵循共享主机部署通用规范（统一目录、健康检查、日志格式、supervisord 安全操作规范）。当用户要求"初始化部署"、"创建部署脚本"、"配置 nginx/supervisor"、"配置 Flyway 数据库迁移"、"新建 Java 工程部署"时触发。支持 /new-java-project -h 查看帮助。
---

# new-java-project

为新工程生成完整的标准化部署配置，包括部署脚本、手工连库入口 `db-sql.sh`、nginx 站点配置、
环境变量模板、Spring Boot 配置（含 Flyway 自动迁移）与健康检查端点、部署规范文档和 `.gitignore`。所有产物严格遵循本 skill 目录下 `specs/deployment-common.md` 的共享主机部署通用规范，
不包含任何硬编码的项目信息。
...
功能
  为 Java/Spring Boot 工程生成完整的标准化部署配置，包括：
    ...
    - scripts/db-sql.sh                    手工连库入口：取连接参数、拼 ssh、起 psql；
                                           默认只读（数据库侧强制 read_only），写只认 --apply，
                                           prod 手工写还要 --prod-approved。**只做数据订正与查询，
                                           不改结构**——结构归 Flyway
    - src/backend/<name>/src/main/resources/db/migration/   Flyway 迁移脚本目录（占位 .gitkeep；
                                           V<n>__<主题>.sql 写在这里，随 jar 打包、入库）
    ...
    - src/backend/<name>/src/main/resources/application.yml       公共配置（端口/地址、时区、
                                           上传上限、JPA、Flyway 自动迁移、Actuator、Swagger 开关）
  ...
  所有产物遵循随本 skill 分发的 specs/deployment-common.md 跨项目通用规范：
    ...
    - 部署日志格式：[YYYY-MM-DD HH:MM:SS] [deploy.sh] <message>，阶段日志 Phase N/M，
      ...
      （db-sql.sh 的日志一律走 stderr，stdout 只出 psql 结果表；结尾同样有 [STATUS] 行）
    ...
    - schema 只有一条写入路径：Flyway（迁移脚本在 classpath:db/migration，应用启动时自动迁移，
      版本记账在目标库的 flyway_schema_history）；Hibernate 只 validate，手工脚本不碰结构
...
生成后自动执行
  chmod +x scripts/deploy.sh scripts/apply-ssl.sh scripts/db-sql.sh
  替换完成后校验：bash -n 逐个脚本（应为替换后的真实值，不再含 <占位符>）
  校验 db-sql.sh 顶部的 SERVICES 表与 deploy.sh 一致
...
注意
  ...
  - Flyway 迁移脚本（src/main/resources/db/migration/*.sql）入库，与代码同版本演进；
    结构变更不需要人工执行步骤 —— 部署（重启进程）本身就是迁移
  - db-sql.sh 只做查询与数据订正，不承担结构变更（避免和 flyway_schema_history 两套记账）
...
src/backend/<SERVICE_NAME>/src/main/resources/db/migration/.gitkeep
sql/README.md
specs/deployment.md
...
> `db/migration/.gitkeep` 是空占位文件，保证迁移目录可提交；工程第一个真实结构变更就是往这里
> 放 `V1__<主题>.sql`（Flyway 在 locations 里找不到脚本时不会报错，空目录是合法的"还没有迁移"）。
...
`templates/scripts/db-sql.sh` 里的 `<SERVICE_NAME>`（`SERVICES` 表与 `-s <SERVICE_NAME>` 示例）
按参数替换，替换后脚本认得的服务名与 `src/backend/<SERVICE_NAME>/` 目录名同源。

### `.gitignore` 生成规则

- 目标工程**没有** `.gitignore` → 按 `templates/gitignore` 模板整体生成（含 Java/Maven、IDE、日志、
  **env 环境变量文件**、**SSL 证书与私钥**、**SQL/数据库备份**、Flyway 迁移脚本入库例外、
...
- 目标工程**已有** `.gitignore` → 不覆盖，仅将模板中缺失的条目合并追加（重点确保：
  ...
  `!src/backend/**/src/main/resources/db/migration/*.sql`（Flyway 迁移脚本要入库），
  `runtime/` / `web-tars/` / `mobile-apps/`），已存在的条目不重复添加。
- 注意：`src/backend/**/.env*` 三份环境变量文件**必须被忽略**（含真实凭证）；
  `application*.yml` 是被跟踪文件，里面只允许 `${VAR:}` 形式的空默认值。
- 注意 `!` 例外的位置：git 无法重新包含"被排除目录里面"的文件，所以工程里若把 `target/` 之外的
  某级源码目录整体排除，迁移脚本的例外会静默失效——追加条目后要用
  `git check-ignore -v src/backend/<服务>/src/main/resources/db/migration/V1__x.sql` 实测一次（应无输出）。
...
- 同步 `scripts/db-sql.sh` 顶部的 `SERVICES` 表：两份脚本的服务名单必须完全一致，否则会出现
  "deploy.sh 能部、db-sql.sh 说这服务不在册"的裂口；同时为新服务建
  `src/backend/<新服务>/src/main/resources/db/migration/` 目录（独立库就独立一套 `flyway_schema_history`）
...
工程**已有自己的结构迁移机制**时，先弄清是哪一种，不能两边并行：

- 已经在用 Flyway（库里有 `flyway_schema_history`）→ 迁移脚本目录沿用工程现有的 `locations`，
  不新建 `db/migration/`；`application.yml` 里只补 `baseline-on-migrate` / `baseline-version`
  这类接管所需的位，`db-sql.sh` 照常生成（它只做查询与订正）
- 用的是**另一套**迁移机制（自研脚本、Liquibase、手工 SQL）→ 问用户要留哪一条。同一个库有两个
  记账来源时，"这条变更到底跑没跑"会有两个互相矛盾的答案，两边都留不如一次定下唯一入口
- 库里已有结构但没有 `flyway_schema_history` → 靠 `baseline-on-migrate: true` +
  `baseline-version: 0` 让 Flyway 补基线，或让用户先手工 `flyway baseline`；两种做法都要在
  `specs/deployment.md` 里写清这次接管把库记成了哪一版，否则下一个 V 文件的编号没有参照
...
```bash
chmod +x scripts/deploy.sh scripts/apply-ssl.sh scripts/db-sql.sh
for f in deploy apply-ssl db-sql; do bash -n "scripts/$f.sh" || echo "语法检查失败: $f"; done
grep -rn '<[A-Z_]\{3,\}>' scripts/ deploy-conf/nginx/ src/backend/<SERVICE_NAME>/   # 应无输出
```
...
```bash
bash scripts/db-sql.sh -h                     # 应打出完整用法（含与 Flyway 的分工说明）
grep -n '^SERVICES=' scripts/deploy.sh scripts/db-sql.sh    # 两份脚本的名单一致
grep -n -A4 'flyway:' src/backend/<SERVICE_NAME>/src/main/resources/application.yml \
    | grep -E 'enabled|locations'             # enabled 默认 true、locations 指 classpath:db/migration
ls src/backend/<SERVICE_NAME>/src/main/resources/db/migration/    # 目录存在（哪怕是空的 .gitkeep）
git check-ignore -v src/backend/<SERVICE_NAME>/src/main/resources/db/migration/V1__x.sql  # 应无输出
grep -rn 'flyway-core' pom.xml src/backend/<SERVICE_NAME>/pom.xml 2>/dev/null \
    || echo "缺依赖：pom.xml 必须显式加 org.flywaydb:flyway-core（+ flyway-database-postgresql）"
```

最后一行是"迁移脚本会不会被 `.gitignore` 的 `*.sql` 吃掉"的实测：被吃掉的话，脚本只活在作者本机，
换个人 clone 出来部署就静默少一次迁移 —— 这条一定要跑，不要靠眼看。
...
2. 准备数据库（如尚未创建）：
   ...
   # 并确认 pom.xml 里有 org.flywaydb:flyway-core（+ flyway-database-postgresql）：
   # 缺依赖时 spring.flyway.* 被静默忽略，一次迁移都不会跑
...
4. 执行部署：
   ...
   # 空库首次启动时 Flyway 建 flyway_schema_history 并跑完 db/migration/ 下的全部脚本，
   # 结构随这次部署一起落到位，不需要单独"建表"步骤

### 此后的结构变更（日常最常走的路径）

1. 写迁移：src/backend/<SERVICE_NAME>/src/main/resources/db/migration/V<n>__<主题>.sql
   （编号接现有最大号往后，不复用不重排；正文写成幂等 SQL）
2. 本地起一次 = dev 库迁移完成，确认落到哪一版（只读）：
   echo 'select version,description,success from flyway_schema_history order by installed_rank desc limit 3' \
       | bash scripts/db-sql.sh -e dev -s <SERVICE_NAME>
3. test / prod 没有额外步骤：**部署本身就是迁移**（应用启动时 Flyway 前滚到最新版本）。
   迁移失败 → 应用起不来 → deploy.sh 的 420s 健康检查判本次部署失败并以非零码中止，
   报错看 /data/logs/apps/<SERVICE_NAME>/supervisord.log
4. 提交时迁移脚本与代码同一个 commit：结构要求与代码要求同源，任何一台机器拉这个 commit
   部署都能得到对得上的库
5. 数据订正（改数据不改结构）才走手工入口，且先只读核对影响行数再放开写：
   bash scripts/db-sql.sh -e test -r <user@host> -f fix.sql            # 先看
   bash scripts/db-sql.sh -e test -r <user@host> --apply -f fix.sql    # 再改
   # 回滚代码不等于回滚结构：Flyway 只前滚，要退结构就再提交一个 V 文件把它改回来
...
6. 该环境的库结构无需单独处理：第 4 步部署重启应用时 Flyway 已把它前滚到最新版本。
   只读核对一下版本（远端只认显式 -r，脚本不会替你连机器）：
   echo 'select version,installed_on,success from flyway_schema_history order by installed_rank desc limit 3' \
       | bash scripts/db-sql.sh -e test -r <user@host> -s <SERVICE_NAME>
   首次接管一个已有表但没有 flyway_schema_history 的库：靠 application.yml 里的
   baseline-on-migrate: true + baseline-version: 0 补基线，别手工建那张表（列不对会让后续校验报错）
...
- **结构变更只有 Flyway 一条路径**：迁移脚本放 `src/main/resources/db/migration/`，应用启动时
  前滚，版本记账只看目标库的 `flyway_schema_history`。不要再引入第二套迁移脚本 —— 同一个库两个
  记账来源时，"这条变更到底跑没跑"会有两个互相矛盾的答案。
- **Flyway 依赖必须由工程 pom 显式声明**（`org.flywaydb:flyway-core`，PostgreSQL 还要
  `flyway-database-postgresql`）：本 skill 不生成 `pom.xml`，而 `spring-boot-starter-data-jpa`
  不会带进 Flyway。缺依赖时 `spring.flyway.*` 整段配置被静默忽略、一次迁移都不会跑，
  空库上表现为启动时 `ddl-auto: validate` 报"表不存在"。生成配置后要在工程 `pom.xml` 里确认这一项，
  并把这条列进交付的人工待办（见第五节）。
- **`db-sql.sh` 不碰结构**：它只做临时查询与数据订正。用它手工 `ALTER` 生产库会让库跑在
  `flyway_schema_history` 记账之外，下次部署 Flyway 校验直接失败（或更糟：静默认为已是新版）。
- **迁移失败就是部署失败**：Flyway 报错 → 应用起不来 → `deploy.sh` 的 420s 健康检查超时中止。
  这是把结构变更绑到部署这个受控动作上的目的，所以 prod 也自动迁移，不留"人记得去跑一次"的口子。
- **不在本 skill 里登记目标机地址**：连哪台机器永远由使用者显式给 `-r <user@host>`。
  `-e test|prod` 只挑 env 文件，本机照它连会连到 dev 库（那两份 env 里的 `127.0.0.1` 指的是
  被部署过去的那台机自己）—— 报告与 dev 一字不差，那是假绿。缺 `-r` 时 db-sql.sh 直接拒。
- **回滚代码不等于回滚结构**：Flyway 只前滚。要退结构就再提交一个 V 文件把它改回来，
  所以迁移脚本里不要做不可逆的数据销毁。
- 本 skill 的 `deploy.sh`/`apply-ssl.sh` 与 `/new-deploy` skill 是同一份副本，
  改动本 skill 下这两个文件时必须同步 `/new-deploy` 那份并用 `md5sum` 校验一致。
  `db-sql.sh` **只由本 skill 生成**（`/new-deploy` 不含数据库层），不是共享副本。
