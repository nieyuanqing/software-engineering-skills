...
| supervisord 程序配置（部署产物，由 deploy.sh 写入） | `/etc/supervisor/conf.d/<SERVICE_NAME>.conf` |
...
# 完全移除（谨慎，仅在确定不再需要时执行）
supervisorctl stop <SERVICE_NAME>
rm /etc/supervisor/conf.d/<SERVICE_NAME>.conf
...
本工程按单服务生成。后续往同一站点加第二个微服务时，需要同时改四处：

1. `scripts/deploy.sh` 顶部服务表：`SERVICES`、`SERVICE_PORTS`、`SERVICE_HEALTH_PATHS`、
   `SERVICE_DBS` 各追加一条（新服务用独立端口，登记到通用规范端口总表）
2. `deploy-conf/nginx/<SERVICE_NAME>.{dev,test,prod}.conf`：为新服务加 `upstream` +
   `location`（按路径前缀分流）
3. `src/backend/<新服务>/.env{,.test,.prod}`：三份键集对齐的环境变量
4. 前端如按端拆分（如管理台 / 门店端两套），把 `deploy.sh` 的 `WEB_APPS` 改成多项并填齐
