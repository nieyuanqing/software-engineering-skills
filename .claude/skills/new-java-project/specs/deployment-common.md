...
部署脚本在（重）启动服务进程后，**不能假设"进程存在=服务已就绪"**就直接往下走（比如接着去装/重载共享的 nginx 配置）。进程起来了不代表服务真的能处理请求——启动期的结构迁移（Flyway 或 `db-migrate.sh` 补的增量）、连接池初始化、缓存预热都需要时间，这段时间里如果就去接流量或者对外宣布"部署成功"，故障会在生产流量打进来的那一刻才暴露。
