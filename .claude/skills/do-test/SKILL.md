...
> **执行方式：必须串行执行，禁止并行。** 同一时刻只允许一个本 skill 实例；测试任务（api / cases）与每个用例均按序逐个执行，上一个用例未出结论前不得开始下一个。禁止并行启动多个实例、并行执行多个用例，也禁止与 /aibug、/aicase 并发运行。
...
- **必须串行执行**：本 skill 全程单实例、任务与用例逐个执行，禁止并行（多实例、多用例同时执行、与 /aibug 或 /aicase 并发均不允许）；用户要求并行时应明确拒绝并说明该约束。
