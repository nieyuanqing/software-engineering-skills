...
选项（通过命令行传入的参数直接使用，不再交互询问）
  ...
  --bucket=N        透传给 /api-test 的 HTTP 请求桶并发大小（整数 1-4）。
                    ...
                    显式传入时以用户值为准，1 = 严格逐条
  --base-url=URL    透传给 /api-test 的后端 API 基础地址（不传由 /api-test 自动探测）
...
3. 若 `test/test-report.md` 已存在，只 grep 提取上一轮汇总行（如 `grep -E "^结果：|^## 三、结论" test/test-report.md`），供本轮报告标注"回归对比"；禁止整份读取报告。
...
1. **干净上下文（防串味）**：子智能体 prompt 只包含——当前用例文件路径（由它自己读取）、base url 与 token（如已有）、`--no-fix` 等执行约束；**禁止**携带之前任何用例的内容、结论、失败现象或已定位原因，主循环也不复用上一个用例的判断。
2. **固定返回格式（≤4 行）**，主循环只记一行台账 `TEST-CASE-<编号> → 通过/失败/阻塞/需人工（一句话原因）`，不追问细节：
...
3. **定期压缩**：每执行完 5 个用例，或感知上下文占用约 60% 时，执行一次 `/compact`，明确要求保留：任务范围与 base url/token、台账表、当前未完成用例的状态。
4. 子智能体返回的失败证据（响应片段/截图说明）一律截断至 **15 行内**，超长不重试、不追问。

对每个用例（传入 `--case` 时只执行匹配项），**以下步骤在子智能体内执行**：
