---
name: common-rules
description: 通用行为规范，适用于所有任务：每次任务完成后输出结果清单、影响范围和开始/结束时间；禁止修改工程根目录 v0/ 下的原始产品设计文档（只读，src/web/v0 代码目录除外）；禁止在代码、配置、脚本中硬编码任何敏感信息；git commit message 必须使用结构化格式；CORS 必须在 nginx 配置中实现，禁止在 Java 后端处理。
---
