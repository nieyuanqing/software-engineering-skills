---
name: common-rules
description: 通用行为规范，适用于所有任务：每次任务完成后输出中文结果清单、影响范围和开始/结束时间（摘要必须采用中文）；禁止修改工程根目录 v0/ 下的原始产品设计文档（只读，src/web/v0 代码目录除外）；禁止在代码、配置、脚本中硬编码任何敏感信息；git commit message 必须使用结构化格式；CORS 必须在 nginx 配置中实现，禁止在 Java 后端处理；上下文窗口达到 70% 后自动执行 /compact。
---
...
规范六：上下文窗口达 70% 自动 /compact
  上下文窗口使用率达到 70% 时，自动执行 /compact 压缩会话历史，
  无需等待用户指示，压缩后继续当前任务。
...
## 六、上下文窗口达 70% 自动 /compact

**触发条件**：上下文窗口使用率达到 **70%**。

**强制行为**：
- 自动执行 `/compact` 压缩会话历史，无需等待用户指示、无需确认
- 压缩完成后继续当前未完成的任务，不丢失任务目标
- 若 /compact 不可用或执行失败，告知用户当前上下文占用情况，建议手动执行 /compact
