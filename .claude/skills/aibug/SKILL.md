---
name: aibug
description: 连接 aibug 系统，循环自动修复 PENDING 状态的 Bug。依次执行：登录获取 token → 获取下一个 Bug → 分析代码并修复 → 更新状态（AI_FIXED/AI_PARTIALLY_FIXED/FAILED），直到无更多待处理 Bug；可用 --bug-id=N[,N…] 只处理指定条目（任意原状态）。必须配合 aibug 系统使用。支持 /aibug -h 查看帮助。
---
...
> **执行方式：必须串行执行，禁止并行。** 同一时刻只允许一个本 skill 实例；**每条 Bug 通过子 agent 处理，严格逐条串行**——上一条 Bug 完成状态回传（AI_FIXED / AI_PARTIALLY_FIXED / FAILED）并回读确认后，才可为下一条 Bug 启动子 agent。禁止并行启动多个实例、并行启动多个子 agent 同时处理不同 Bug，也禁止与 /aicase、/do-test 并发运行。

---

## 零、参数处理

**如果用户传入 `-h` 或 `--help`**，不执行任何操作，直接把下面的帮助信息**原样输出在本次回复正文里**后结束
——斜杠命令把 SKILL.md 注入我的上下文**不等于**已展示给你，正文里只回一句"已输出/已同步"就是没输出。
`-h` 与其它参数或说明文字同时出现时，一律**只出帮助、忽略其余参数**：本轮不猜附加要求，需要同时执行就把
`-h` 去掉分两次调用。
...
工作流程
  ...
  3. PUT  {host}/aibug/api/bugs/{id}/status    标记为 IN_PROGRESS（队列与指定模式都执行）
...
Bug 字段说明
  ...
  failReason  失败原因（状态为 FAILED 时必填），固定三行纯文本：
              现象 / 定位 / 下一步
  fixNote     部分修复说明（状态为 AI_PARTIALLY_FIXED 时必填），固定三行纯文本：
              已修复 / 待修复 / 验证
...
**请求体禁止内联凭据（必做）**：`--password` 传进来的值**先落到 shell 变量**，再交给程序组装，**禁止**把口令
字面量写进 `curl` 的 `-d` / `--data-raw` / URL，即命令行里不得出现 `"password":"<值>"` 这种连写形态。命令执行链
上的凭据脱敏会把该形态的值改写成 `***`（3 字符），服务端按 BCrypt 正常拒绝，返回 **401 + `用户名或密码错误`**
——文案与"真的口令错"完全一致，从响应体无从区分。请求体一律在程序内用 `json.dumps` 组装：

```bash
python3 - "$HOST" "$USERNAME" "$PASSWORD" <<'PY'
...
- **判为"凭据问题"之前，必须先做传参自检**（三项全过才可以对用户说口令有误，否则就是假阴性死循环——用户给对
  ...
  ② 换一种**不经 shell 引号与脱敏**的发送姿势复测（按上面程序内组装 body 的 urllib 写法）；
  ③ 两种姿势结果不一致 → 结论是**本地传参链路问题**：改用程序内组装 body 的姿势重发，**不得**向用户索取口令、
...
以下步骤循环执行，直到无更多 PENDING Bug 为止。**指定模式（传了 `BUG_IDS`）按清单逐条走一遍**：每个 `#bugId` 都完成 3.3 判定并经 3.4 回读确认后才取下一个，全部走完即结束整个流程，不回队列。
...
[回传状态]   PUT /aibug/api/bugs/{id}/status → 200，status=AI_PARTIALLY_FIXED，fixNote=已修复/待修复/验证
[回读确认]   GET /aibug/api/bugs/{id} → 200，status=AI_PARTIALLY_FIXED，fixNote 三行逐行一致
```
...
- 全文示例里的 `{id}` 与 `{CURRENT_ID}` **同义**，都指当前 Bug 卡的 `id`，不存在第二个 ID 变量（参数解析里的清单变量统一叫 `BUG_IDS`，处理到哪一条，`{id}` 就是哪一条）
...
1. **子智能体隔离**：3.3 的分析与修复默认委托给子智能体（Agent 工具）执行——子智能体拥有独立上下文，定位/阅读/修改代码的完整过程不进入主循环；主循环只接收结构化结果（常规 4 行，`AI_PARTIALLY_FIXED`/`FAILED` 追加 3 行回写字段）。
2. **响应瘦身**：所有 API 响应只保留 `id`、`content`（≤500 字，超长截断）、`fileUrls`、`status`、`fixNote`、`error` 字段；禁止把完整 JSON 原文粘进对话。
3. **台账式记录**：主循环只维护一行式台账 `#<id>（原状态 <X>）→ AI_FIXED/AI_PARTIALLY_FIXED/FAILED（AI_FIXED 中本轮零改动的已修复/重复修复追加标注 `AI_FIXED（已修复，无改动：<文件:行>）`、AI_PARTIALLY_FIXED 摘 fixNote 的"待修复"行、FAILED 摘 failReason 的"定位"行）`，异常终止写 `#<id>（原状态 <X>）→ 异常终止，卡在 IN_PROGRESS`。指定模式（`--bug-id`）里"原状态"与取卡时的 `fixNote`/`failReason` **原文**是用户回退的唯一依据，必须逐字保留（过长可另起一行按 `#<id>` 挂在台账下方，禁止只写"已摘要"）；其余情形不保留分析细节，也不在台账里重复整段回写字段。
...
- **不限当前状态**：`PENDING / IN_PROGRESS / AI_FIXED / AI_PARTIALLY_FIXED / FAILED / RESOLVED / CLOSED` 均可重新处理，本轮终态覆盖原状态；覆盖前先向用户明示 `#<id> <原状态> → <本轮判定状态>`，并把原状态与原有 `fixNote`/`failReason` **原文**记入台账（服务端在其它状态下会清空说明字段，落库后不可恢复）；3.2 的预标记一旦发生，本轮任何异常终止都会让该条停在 `IN_PROGRESS`、脱离 `/bugs/next` 队列，善后见 3.2 末与第四节"异常终止"清单；
- 该条处理完（3.4 回读确认通过）后，清单里还有未处理 ID 就取下一个，否则进入完成输出，**不回到本步拉队列**。
...
获取到 Bug 后（队列模式与指定模式一律，含 `--bug-id` 清单里的每一条），立即更新状态，防止被重复领取：
...
- **预标记后的异常终止善后（必做）**：本轮无论因何中止（用户中断、3.3 判不出结果、token 重登后仍 401、上面这条回读两次不一致），该条都会停在 `IN_PROGRESS`——队列模式下它不再被 `/bugs/next` 下发，指定模式下它已盖掉 `CLOSED`/`RESOLVED`/`AI_FIXED` 等原状态。台账与第四节"异常终止"清单必须写明 `#<id> 卡在 IN_PROGRESS，原状态 <X>，fixNote/failReason 原文 <台账原文>`，提示用户到 aibug 界面改回；**不得**自行把它改回原状态或 `PENDING`（本 skill 只回写 `AI_FIXED` / `AI_PARTIALLY_FIXED` / `FAILED`），也不得臆造一条新的 PUT。
...
要求子智能体以下列固定格式返回（常规 4 行，`修改文件` 可占多行；结果为 `AI_PARTIALLY_FIXED` / `FAILED` 时必须追加对应的 3 行结构化回写字段），主循环只把结论记入台账，并按原样把回写字段提交给服务端，**禁止改写成一段连续文字**：

```
结果: AI_FIXED | AI_PARTIALLY_FIXED | FAILED
修改文件: <逐行列出；FAILED 写 无；已修复/重复修复写 无（已存在修复：<文件:行 或 接口>）>
验证: <验证方式> → 通过 | 失败
说明: <一句话根因>
回写字段(fixNote): <仅 AI_PARTIALLY_FIXED，按 3.4 三行结构：已修复 / 待修复 / 验证>
回写字段(failReason): <仅 FAILED，按 3.4 三行结构：现象 / 定位 / 下一步>
```

`结果` 为 `AI_FIXED` 时两个回写字段都省略；缺行、标签改名、把多点写成一段散文、或字段值里带 markdown 标记（加粗/行内代码/表格/引用/行首 `#`、`-`、`*`、`数字.`/空行/代码块围栏），一律视为格式不合规退回子智能体按结构重发，主循环不得自行拼句子补全或直接提交。

**三态判定标准**（写入子智能体 prompt，必须逐点核对）：把 `content` 拆成独立问题点逐条核对——

- 全部问题点均已修复且验证通过 → `AI_FIXED`
- 仅部分问题点修复（如只覆盖一端/一条路径/一个接口，其余点未处理或需外部系统、需产品决策配合）→ `AI_PARTIALLY_FIXED`，并按 `回写字段(fixNote)` 写满三行，`已修复` 行必须是本轮验证通过的内容
- 该改的一点没改、或修复后验证不通过 → `FAILED`，并按 `回写字段(failReason)` 写满三行（本轮无改动但复验确认原现象已消失的，按下方"已修复 / 重复修复"判 `AI_FIXED`，不归入本条）

禁止把已全量修好且验证通过的 Bug 报成 `AI_PARTIALLY_FIXED`（避免虚增人工待办），也禁止用 `AI_PARTIALLY_FIXED` 掩盖验证失败。

**已修复 / 重复修复**（写入子智能体 prompt，与三态同级判定）：定位后发现所报问题点在现有代码中**已经修复**（历史提交已覆盖、上一轮修复后状态回传失败被重新领取、同一问题被重复报单），本轮无需任何改动时，判定只看**按原现象的实测复验**，与常规验证方式相同（编译/构建/接口 curl/相关测试）——

- 复验确认 `content` 描述的现象**已不再出现** → `结果: AI_FIXED`、`验证: <验证方式> → 通过`，`修改文件` 行写 `无（已存在修复：<文件:行 或 接口>）`，`说明` 行写清来源（`历史提交已覆盖` / `与 #<id> 重复报单` / `上一轮未回传状态`）；**禁止**因"本轮没改代码"报 `FAILED` 或 `AI_PARTIALLY_FIXED`
- 复验仍能复现现象 → 不算已修复，回到上面三态正常判定（该改的改，改完复验；确实修不了才 `FAILED`）
- 既无法复现也无从验证（缺环境/缺数据/必须人工操作）→ 不视为已修复，按 `FAILED` 走，`定位` 行写明阻塞点

同一问题点被报成多条 Bug 时，每条按自己的 `#id` 独立回传 `AI_FIXED`（服务端各条状态互不影响），不做"只回传一条、其余跳过"。

子智能体内部执行：理解 `content` → 查看附件图片（如有）→ 定位代码 → Edit/Write 最小化修复 → **即刻验证（必做）**：修复完成后**在同一轮内立即验证**，验证通过才返回结果，然后才进入 3.4 更新状态、再取下一个 Bug；**禁止**把验证推迟到所有 Bug 修复完后统一做。验证方式按工程类型选最低成本——Java 工程 `mvn -q -DskipTests compile` 编译通过；前端工程构建或 lint 通过；Bug 指向具体接口时用 curl 复测该接口行为符合描述预期；工程有相关测试则运行对应测试。验证未通过则继续修复直至通过，仍无法通过则如实报告 `验证: 失败`，不得虚报 AI_FIXED。
...
| 子智能体结果 | 回传状态 | 必填说明字段 |
|---|---|---|
| `结果: AI_FIXED` + `验证: 通过`，问题点全部覆盖 | `AI_FIXED` | 无 |
| `结果: AI_FIXED` + `验证: 通过`，本轮零改动（已修复/重复修复） | `AI_FIXED` | 无 |
| `结果: AI_PARTIALLY_FIXED` + 已修复部分 `验证: 通过` | `AI_PARTIALLY_FIXED` | `fixNote` |
| `结果: FAILED`，或复验仍不通过 | `FAILED` | `failReason` |

**标记 AI_FIXED 的前提**：该 Bug 的验证已在**本轮修复后即刻完成**（不是全部修完后统一验证），且子智能体结果同时满足 `结果: AI_FIXED` 与 `验证: 通过`。**已修复 / 重复修复同样满足该前提**——本轮虽无代码改动，但按 `content` 原现象做的复验就是本轮验证，通过即按该 Bug 的 `#id` 正常回传 `AI_FIXED`（复用下方"全部修复"的 PUT 命令），不得标 `FAILED`、也不得为"留痕"改成 `AI_PARTIALLY_FIXED`（服务端 `AI_FIXED` 状态不保存说明字段，判据只留在台账与本轮汇总）。若 `验证: 失败`，退回子智能体追加修复一轮（携带失败现象），复验仍失败则标记 `FAILED`，`failReason` 按下文三行结构填写，`现象` 行写"修复后复验未通过：<验证方式与失败现象>"。
...
curl -s -X PUT "{HOST}/aibug/api/bugs/{id}/status" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"status":"AI_FIXED","projectId":{PROJECT_ID}}'
```

**部分修复** → 标记为 `AI_PARTIALLY_FIXED`，并填写 `fixNote`（必填，缺失服务端返回 400）：

```bash
curl -s -X PUT "{HOST}/aibug/api/bugs/{id}/status" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"status":"AI_PARTIALLY_FIXED","projectId":{PROJECT_ID},"fixNote":"已修复：<…>\n待修复：<…>\n验证：<…> → 通过"}'
```
...
- 每个 `AI_PARTIALLY_FIXED` 的 Bug，要把 `待修复` 那一行原样列入本次任务的**人工待办**清单（common-rules 规范一"人工待办"板块），标注 `#<id>`，`@角色` 按该行点名的归属填写，未点名时默认 `@研发`

**回读验证**（必做）：PUT 后用与 3.2 相同的方式回读 `GET /bugs/{id}`，确认 `status` 为 `AI_FIXED`、`AI_PARTIALLY_FIXED` 或 `FAILED`；标为 `AI_PARTIALLY_FIXED` / `FAILED` 时还要逐行确认回读的 `fixNote` / `failReason` 与提交内容一致、三个标签行齐全未塌成一行。服务端存的是提交原文（markdown 只在展示端渲染），逐行比对按原文即可；带链接的字段，另在 aibug 查看弹窗确认链接可点开、没有被解析成标题/列表/代码块。不一致则重试一次，仍不一致则向用户告警，并保留该 Bug 的 ID、期望状态与缺失或被改动的标签行。

> 服务端只在对应状态下保存说明字段：`failReason` 仅 `FAILED` 保留，`fixNote` 仅 `AI_PARTIALLY_FIXED` 保留，其余状态一律置空。因此后续把某条 `AI_PARTIALLY_FIXED` 的 Bug 改成 `AI_FIXED`/`RESOLVED` 时原 `fixNote` 会被清空，需要留痕的剩余问题应先另建 Bug 或在本轮汇总中记录。

### 3.5 循环至下一个 Bug

完成一个 Bug 的处理后，回到 **3.1**，继续获取下一个 PENDING Bug。**指定模式（传了 `BUG_IDS`）不回队列**：清单里还有未处理的 `#bugId` 就按顺序取下一个（各自回写各自的 `#id`），全部处理完直接进入第四节。
...
共处理 Bug：N 个
  - AI_FIXED：N 个（其中已修复/重复修复、本轮零改动 m 个：逐条列出 #id 与已存在修复位置；无则 0）
  - AI_PARTIALLY_FIXED：N 个（逐条列出 #id 与 fixNote 的 `待修复` 行原文；无则 0）
  ...
  - 异常终止（卡在 IN_PROGRESS）：N 个（逐条列出 #id、取卡时的原状态、台账记下的 fixNote/failReason 原文；无则 0）
...
- `AI_PARTIALLY_FIXED` 的 Bug 已脱离 PENDING 队列（服务端 `/bugs/next` 只下发 `PENDING`），本轮及后续 `/aibug` 都不会再自动处理其剩余问题；需在 aibug 界面把状态改回 `PENDING` 才会重新进入队列。
- 卡在 `IN_PROGRESS` 的条目同样脱离队列，且指定模式下原状态已被 3.2 的预标记盖掉、说明字段已被服务端清空；只能按台账记录到 aibug 界面人工改回原状态或 `PENDING`，本 skill 不回写这两个状态。
...
- 每次修复前先标记 `IN_PROGRESS`（队列与指定模式都执行），确保同一 Bug 不被并发处理；处理完毕必须按 3.4 回写终态并回读确认。
- **预标记的代价**：`IN_PROGRESS` 一旦写入，该条就脱离 `/bugs/next` 队列，指定模式下还会盖掉 `CLOSED`/`RESOLVED`/`AI_FIXED` 等原状态并连带清空 `fixNote`/`failReason`（服务端规则，落库不可恢复）。本轮异常终止时必须按台账记下的原状态与原文在汇总的"异常终止"清单里点明，交人工在 aibug 界面回退；本 skill 不回写 `PENDING`/`RESOLVED`/`CLOSED`，不得自行善后。
