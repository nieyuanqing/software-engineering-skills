...
示例
  ...
  /new-macos-build -h    显示本帮助（出现即忽略其它参数，只出帮助）
```
...
以下路径均相对于**本 skill 目录**（安装后通常为 `~/.qoder/skills/new-macos-build/`），模板随 skill 一起分发，不依赖任何仓库克隆或本地工程。

| 目标文件 | 模板来源 |
|---|---|
| `scripts/macos-build.sh` | `templates/scripts/macos-build.sh` |
