...
**如果用户传入 `-h` 或 `--help`**，不执行任何比对，直接把下面的帮助信息**原样输出在本次回复正文里**后结束（斜杠命令把 SKILL.md 注入我的上下文不等于已展示给用户，正文里只回一句"已输出"就是没输出）。`-h` 与其它参数或说明文字同时出现时，一律**只出帮助、忽略其余参数**：本轮不猜附加要求，需要同时执行就把 `-h` 去掉分两次调用：
...
1. **`--dst-db` 缺失必须先提醒再停**：向用户输出
   ...
   然后结束本轮，不猜测主机、不读任何库、不用当前项目的 `src/backend/<服务>/.env.prod` 顶替。
