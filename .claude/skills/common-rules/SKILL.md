...
**触发条件**：与规范一联动——凡输出任务摘要（涉及文件操作或系统变更）的任务，推送一条飞书通知；纯对话/查询类请求不推送。
...
| 影响范围 | `elements[3]` `div.text` | `**影响范围**` + 固定子项：`新增文件`、`修改文件`、`删除文件`、`数据库变更`；无内容的子项写 `无`，取不到值的子项可省略 |
| 人工待办 | `elements[4]` `div.text` | `**人工待办**` + 取规范一同名板块逐条照搬：每条一行、行首 `@角色`（`@研发`/`@运维`/`@DBA`/`@产品`…）、≤5 条；无待办时写 `- 无`，区块不得省略 |
...
- **回退**：卡片因格式/模板问题返回非 0（如 `code=19024` 等）时，用同样字段与顺序的固定行文本模板（`msg_type: "text"`）重发一次，字段顺序：标题行 → `结论` → `开始/结束/耗时` → `任务结果` → `影响范围`（新增/修改/删除文件、数据库变更）→ `人工待办`（逐条 `@角色` 前缀，无则写"无"）；除此之外不重复发送，网络超时不重试刷屏
