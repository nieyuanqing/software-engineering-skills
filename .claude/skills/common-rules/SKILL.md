...
选项
  --feishu-webhook=URL   飞书群机器人 Webhook 地址，激活规范九（任务完成发送通知）
...
规范九：任务完成后发送飞书通知
  ...
  参数配置（激活本 skill 时通过命令行传入，仅在会话内持有、跨任务复用；
  禁止写入任何文件——代码/配置/脚本/提交记录，与规范三联动）：
    - --feishu-webhook=URL    群自定义机器人 Webhook 地址（必传，不传则本规范不生效）
    - --feishu-secret=CODE    机器人"加签"密钥（可选，机器人开启加签时必传）
  发送规则：
    - 未传 --feishu-webhook 则不发送，静默跳过，不报错、不阻断任务
...
**参数配置**（`/common-rules` 激活时通过命令行参数传入，激活期内仅在会话中持有、跨任务复用；禁止写入任何文件，与规范三联动）：

| 参数 | 必填 | 说明 |
|---|---|---|
| `--feishu-webhook=URL` | 是 | 飞书群自定义机器人 Webhook 地址；未传入则本规范不生效，静默跳过 |
